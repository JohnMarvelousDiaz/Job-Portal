    ...
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            if user is not None:
                ...
                if user1.type == "applicant" and user1.status == "Activated":
                    login(request, user)
                    return redirect("/user_homepage/")
            else:
                thank = True
...
def index(request):
    jobs = Job.objects.all().order_by('-start_date')
    apply = Application.objects.filter()
    data = []
    for i in apply:
        data.append(i.job.id)
...
def signup(request):
    passnotmatch = False  # Define passnotmatch with an initial value
    ...
    if request.method == "POST":   
        ...
        applicants = Applicant.objects.create(user=user, phone=phone, gender=gender, image=image, type="applicant", status="Activated")
        user.save()
        applicants.save()
        
        signup = True
        return render(request, "user_login.html", {"signup": signup, "passnotmatch": passnotmatch})
...
def company_signup(request):
    if request.method=="POST":   
        ...
        alert3=True
        return render(request, "company_login.html",{"alert3":alert3})
...
def company_login(request):
    if request.method == "POST":
        ...
        if user is not None:#login company homepage
            ...
            if user1.type == "company" and user1.status != "pending" and user1.status != "Rejected":
                ...
                alert2=True
                return render(request, "company_login.html", {"alert2":alert2})
...
def applicant_status(request, myid):
    if not request.user.is_authenticated:
        return redirect("/admin_login/")
    applicants = Applicant.objects.get(user_id=myid)
    if request.method == "POST":
        status = request.POST['status']
        applicants.status=status
        applicants.save()
        alert = True
        return render(request, "applicant_status.html", {'alert':alert})
    return render(request, "applicant_status.html", {'applicants':applicants})

def activated_applicant(request):
    if not request.user.is_authenticated:
        return redirect("/admin_login/")
    applicants = Applicant.objects.filter(status="activated")
    return render(request, "activated_applicant.html", {'applicants':applicants})

def deactivated_applicant(request):
    if not request.user.is_authenticated:
        return redirect("/admin_login/")
    applicants = Applicant.objects.filter(status="deactivated")
    return render(request, "deactivated_applicant.html", {'applicants':applicants})
...
def delete_applicant(request, myid):
    ...
    applicant.delete()
    alert=True
    return redirect("/view_applicants/",{'alert', alert})
...
def search_job(request):
    if request.method=="POST":
        ...
        jobs = Job.objects.filter(Q(title__contains=searched) | Q(location__contains=searched) | Q(skills__contains=searched) )
        return render(request, "search_job.html", {'searched': searched,'jobs': jobs})
    else:
